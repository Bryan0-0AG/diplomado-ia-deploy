...
def obtener_graficos_analisis_target(df, columna_target="Factura Ahorrada Mensual"):
    ...
    if columna_target in df.columns:
        ...
        ax_box.set_xlabel(columna_target)
        ax_box.grid(True)
        figs.append(fig_box)
...
# Funcion original para mantener compatibilidad con main.py
def generar_graficos(df):
    figs_cat = obtener_graficos_categoricos(df)
    for fig in figs_cat:
        plt.show()
    
    figs_num = obtener_graficos_numericos(df)
    for fig in figs_num:
        plt.show()
    
    figs_target = obtener_graficos_analisis_target(df)
    for fig in figs_target:
        plt.show()
    
    plt.show(obtener_graficos_correlacion(df))
    plt.show(obtener_grafico_nulos(df))
